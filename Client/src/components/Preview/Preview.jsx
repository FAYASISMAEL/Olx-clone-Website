      ...
      return (
        <div className="w-full md:w-72 lg:w-80">
          <div className="bg-white p-4 sm:p-6 rounded-lg shadow mb-4">
            ...
            <button className="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700 mb-4 text-sm sm:text-base">
              Make Offer
            </button>
            <div className="flex items-center mb-4">
              <img
              ...
              />
              <div>
                <p className="font-semibold text-sm sm:text-base">Unknown</p>
            ...
            <button className="w-full border border-blue-600 text-blue-600 py-2 rounded hover:bg-blue-50 mb-4 text-sm sm:text-base">
              Chat with Seller
            </button>
            <div className="flex items-center">
              <svg
...
// const ImageCarousel = ({ images, currentImage, onPrev, onNext, serverUrl, adId, ad, setAd }) => {
//   const { likedAds, toggleLike } = useLikes();
...
//     if (!email) {
//       toast.error('Please log in to like ads.', {
//         position: 'top-right',
//         autoClose: 3000,
//       });
//       return;
...
//     const prevAd = { ...ad };
//     const prevLikedStatus = likedAds[adId] || false;

//     // Optimistically update the UI
//     const currentLikes = Array.isArray(ad.like) ? ad.like : [];
//     const isCurrentlyLiked = currentLikes.includes(email);
//     const newLikes = isCurrentlyLiked
//       ? currentLikes.filter(e => e !== email)
//       : [...currentLikes, email];

//     setAd(prev => ({ ...prev, like: newLikes }));
//     toggleLike(adId);

//     try {
//       const response = await axios.post(`${serverUrl}/api/like/${adId}`, { email });
//       const { message, post } = response.data;
//       console.log('Like response:', { adId, message, likeArray: post.like });

//       // Update ad state with server response
//       const serverLikes = Array.isArray(post.like) ? post.like : [];
//       setAd(prev => ({ ...prev, like: serverLikes }));
//       // Ensure likedAds is consistent with server
//       if (serverLikes.includes(email) !== likedAds[adId]) {
//         toggleLike(adId);
//       }
...
//       });
//     } catch (error) {
//       console.error('Error liking/unliking ad:', error);
//       // Revert optimistic update
//       setAd(prevAd);
//       toggleLike(adId); // Revert context state
...
//           >
//             <svg
//               className={`w-5 h-5 sm:w-6 sm:h-6 ${likedAds[adId] ? 'text-red-400 fill-red-400' : 'text-white'}`}
//               fill={likedAds[adId] ? 'currentColor' : 'none'}
//               stroke="currentColor"
//               viewBox="0 0 24 24"
...
//   const getPostedTime = (postedDate) => {
//     const currentDate = new Date("2025-05-15T17:13:00+05:30"); // 5:13 PM IST
//     const posted = new Date(postedDate);
//     const diffMs = currentDate - posted;
//     const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));
//     const diffHours = Math.floor((diffMs % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
...
//       const fetchedAd = {
//         ...response.data,
//         like: Array.isArray(response.data.like) ? response.data.like : [],
//       };
//       setAd(fetchedAd);
...
//         setLikedAds(prev => ({
//           ...prev,
//           [adId]: fetchedAd.like.includes(email),
//         }));
//       }
//     } catch (err) {
//       console.error("Error fetching ad:", err);
